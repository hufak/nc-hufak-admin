...
const props = withDefaults(
	defineProps<{
		...
		readDefaultValue: (data: Record<string, unknown>) => string
...
const hasChanges = computed(() => value.value !== initialValue.value);
...
onMounted(async () => {
	try {
		...
		defaultValue.value = props.readDefaultValue(data);
		status.value = '';
	} catch (err) {
...
<template>
	<section :style="styles.formSection">
		...
		<form :style="styles.form" @submit.prevent="save">
			...
			<SettingEditorActions :save-label="`Save ${settingName}`" :loading="loading" :saving="saving" :has-changes="hasChanges" :can-save="canSave" @reset="reset" @load-defaults="loadDefaults" />
