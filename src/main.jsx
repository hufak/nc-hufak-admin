...
const SECTIONS = [
	...
	{ key: SECTION_KEYS.CONFIGURE_MAIL, label: 'set user mailbox' },
...
function AddUser({ emailDomain }) {
	...
	const onSubmit = async (event) => {
		...
		try {
			...
			const lines = [`✅ Step 1/${totalSteps}: ${data.message || `User "${actualCreatedUid}" created successfully`}`];
			if (data.password) {
			...
			if (shouldConfigureDefaultMailbox) {
				...
				try {
					...
					lines.push(`✅ Step 3/${totalSteps}: Primary user mailbox configured. ${messageParts.join(' | ')}`);
				} catch (step3Err) {
					lines.push(
		...
		<section style={styles.formSection}>
			...
			<form onSubmit={onSubmit} style={styles.form}>
				...
				<input
					...
					onChange={(event) => setDefaultEmailAccount(event.target.value)}
				...
				<input
					...
					onChange={(event) => setDefaultEmailAccountPassword(event.target.value)}
	...
	return (
		<section style={styles.formSection}>
			...
			<ul style={styles.overviewList}>
				...
				<li>
					<strong>set user mailbox</strong>: select a user and set the primary SnappyMail
								...
								<tr key={user.uid}>
									...
									<td style={styles.tableCell}>
										<div
											...
											onMouseLeave={() => setHoveredUid((current) => (current === user.uid ? '' : current))}
										>
											...
											{hoveredUid === user.uid && (
												<div style={styles.tooltipPanel}>
													...
													>
														{resettingUid === user.uid ? 'Setting...' : 'set to defaults'}
													</button>
												</div>
											)}
										</div>
